...
struct ContentView: View {
    ...
    @State private var editingEntry: FareboxEntry?
    ...
    private var emptyState: some View {
        VStack(spacing: 12) {
            ...
            Text("No entries yet")
                ...
                .foregroundStyle(Theme.textPrimary)
            Text("Log rideshare and taxi fares to track real transportation spend.")
                .font(Theme.captionFont)
                .foregroundStyle(Theme.textSecondary)
    ...
    private func row(for entry: FareboxEntry) -> some View {
        HStack {
...
struct EntryEditView: View {
    ...
    var onSave: (FareboxEntry) -> Void

    private let existingID: UUID?

    init(store: Store, existing: FareboxEntry? = nil, onSave: @escaping (FareboxEntry) -> Void) {
        self.store = store
        self.onSave = onSave
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        ...
                        let entry = FareboxEntry(id: existingID ?? UUID(), title: title.isEmpty ? "Untitled" : title, amount: amount, date: date, note: note)
                        onSave(entry)
                        dismiss()
                    }
